...
//   hal.console->printf("_^^^^^^^^_\r\n");
//   hal.console->printf("_num_sensors=%d\r\n",_num_sensors);
//   hal.console->printf("+^^^^^^^^+\r\n");
    ...
    for (uint8_t i=0; i<_num_sensors; i++)
    {
        ...
        if (sensors[i].healthy)
        {
            ...
            if (sensors[i].type == BARO_TYPE_AIR) //sensors[i].type=0
            {
                altitude = get_altitude_difference(sensors[i].ground_pressure, corrected_pressure);
//                Baro_altitude[i]=altitude;
//                hal.console->printf("i=%d\r\n",i);
//                hal.console->printf("Baro_altitude[i]=%d\r\n",Baro_altitude[i]);
